...
class QForm_Element
{
    ...
    function delValidations($str)
    {
        foreach($this->_validations as $key => $test)
        {
            if($test[0] == $str)
            {
                unset($this->_validations[$key]);
            }
        }
    ...
    function errorString($de = ';')
    {
        if($this->errorMsg())
        return implode($de,$this->errorMsg());
    }

    function tips($def='',$de = ';')
    {
        if($this->errorMsg())
        return implode($de,$this->errorMsg());
        else return $def;
    ...
    function label($def = null)
    {

        return $this->label ?  $this->label : ($this->_label ? $this->_label:$def);
    }

    function _label($def = null)
    {

        return !empty($this->label) ?  $this->label : ($this->_label ? $this->_label:$def);
    }
