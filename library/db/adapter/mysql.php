...
class QDB_Adapter_Mysql extends QDB_Adapter_Abstract
{
    ...
    function connect($pconnect = false, $force_new = false)
    {
        ...
        if ($pconnect)
        {
            $this->_conn = mysqli_connect('p:' . $host, $this->_dsn['login'], $this->_dsn['password'], $this->_dsn['database']);
        }
        else
        {
    ...
    function execute($sql, $inputarr = null, $tryCount = 0)
    {
        ...
        switch (true)
        {
            case stripos($sql, 'DELETE FROM') !== false:
            #case stripos($sql, 'INSERT INTO') !== false:
            case stripos($sql, 'UPDATE `') !== false:
            #__('写分离');
            if(isset($this->_dsn['host_w']) && $this->_dsn['host_w'])
            {
                $conn = mysqli_connect($this->_dsn['host_w'], $this->_dsn['login'], $this->_dsn['password'], $this->_dsn['database']);
            }
            break;
        }
        ...
        {
            $this->_last_err         = mysqli_error($conn);
            $this->_last_err_code    = mysqli_errno($conn);
            ...
            else
            {
                if(PHP_CLI)
                {
                    if ($this->_conn->errno == '2006' && $tryCount < 2)
                    {
                        //__('mysqlError:' . $this->_conn->errno . '#count->' . $tryCount);
                        $tryCount++;
                        return $this->execute($sql, $inputarr, $tryCount);
                    }else
                    {
