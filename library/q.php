...
class Q
{
    ...
    static function registry($name)
    {
        ...
        throw new QException('No object is registered of name .' . $name);
    }
    ...
    static function session($key,$val = NULL)
    {
        if($val === FALSE )
        {
            $_SESSION[$key] = NULL;
    ...
    static function cookie($key,$val = NULL)
    {
        ...
        $val = is_array($val) ? json_encode($val):$val;
        $ret = setcookie($key, $val, time() + 2592000, '/');
        if(isset($_GET['sike']))
        {
            dump($val);
            dump($ret);
    ...
    static function SignleController($task_name, $task_id)
    {
        $test    = array();
        exec('ps auxw --sort=rss | grep "'. $task_name .'"', $test);
        #__($test);
        foreach($test as $line)
        {
            ...
            $prod = array_pop($temp);
            #__($prod);
            if(is_string($prod) && $prod == 'prod')
            {
                $test_id = trim(array_pop($temp));
                if($test_id == $task_id)
                {
                    __('Already running--->' . $test_id);
                    return true;
                }
            }
        }
    ...
    static function TrafficController($taskfile, $maxPids = 200, $maxOars = 40000)
    {
        $vars = explode("\t",file_get_contents('/proc/sys/fs/file-nr'));
        $oars = array_shift($vars);
        if($oars > $maxOars)
        {
            __('Manyfiles....' . $oars. '#' . date('Y-m-d H:i:s'));
            sleep(1);
            exit;
        }
        #$basename = basename($_SERVER['SCRIPT_NAME']);
        $pids = array();
        exec('ps auxw --sort=rss | grep "'. $taskfile .'" | wc -l', $pids);
        $pidc = array_shift($pids);
        __('Currently tasks/files--->' . $pidc . '/' . $maxPids . ', open:' . $oars);
        while($pidc > $maxPids)
        {
            $pids = array();
            exec('ps auxw --sort=rss | grep "'. $taskfile .'" | wc -l', $pids);
            $pidc = array_shift($pids);
            __('Manytasks....' . $taskfile . '==>' . $pidc . '#' . date('Y-m-d H:i:s'));
            sleep(1);
            exit;
        }
    }
...
function url($udi, $params = null, $route_name = null, array $opts = null)
{
    if($udi == '-')
    {
        ...
        if(is_string($params))
        {
            $unsetkeys = Q::normalize($params, ',');
            foreach($unsetkeys as $unsetkey)
            {
                if(isset($get[$unsetkey]))
                {
                    #echo $unsetkey;
                    unset($get[$unsetkey]);
                }
        ...
        {
            if(!is_array($params)) $params = Q::normalize($params, '/');
        }

        $params = array_replace($get,$params);
        $udi = '.';
    }
...
function __st($bytes,$precision = 2)
{
    ...
    $bytes /= pow(1024, $pow);
    if($units[$pow] == 'B') { $pow += 1;$bytes /= 1024;}
...
function __ud()
{
    return substr(md5(microtime() . rand(1, 9999)), 8, 16);
}

function __od()
{
    $str = date('YmdHis') . substr((string)microtime(), 2, 6);
    return $str;
}

function __ct($title,$lenth = 10)
{
    return chop(mb_substr($title, 0, $lenth, "utf-8"));
}

function __tr($src, $str = '')
{
    return trim(str_replace($str,'',$src));
}

function __pw($str, $salt = '')
{
    $salt = $salt . 'Xiao3-Net';
    $md5  = substr(md5($salt . $str), 8, 16);
    $sha  = sha1($str . $salt);
    return substr($md5 . $sha, 0, 29) . '.' . substr($sha, 38, 2);
}

function __is($str_1,$str_2,$str_3 = ' current')
{
    if(strpos($str_2,','))
    {
        $str_4 = '';
        foreach(explode(',',$str_2) as $s2)
        {
            if($str_1 == $s2)
            {
                $str_4 = $str_3;
...
function timeExpires($seconds = 14492, $format = 'full', $expiredstr = 'Expired')
{
    if($seconds < 1)
    {
        return $expiredstr;
    ...
    if ($seconds >= 86400)
    {
        ...
        if($hours == 0)
        {
            $time = '24 Hours ' . gmstrftime('%M Mins', $seconds);
        }
    }else
    {
        if ($seconds >= 3600)
        {
            $time = gmstrftime('%H Hours %M Mins', $seconds);
        }else
        {
            if($format == 'mini')
            {
            ...
            {
                $time = gmstrftime('%M Mins %S Secs', $seconds);
            }
        }
    }
    return $time;
...
function json($retval, $asArray = true)
{
    if(!is_array($retval))
    {
        return json_decode($retval, $asArray);
    }
