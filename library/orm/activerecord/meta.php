...
class QDB_ActiveRecord_Meta implements QDB_ActiveRecord_Callbacks
{
    ...
    static function instance($class)
    {
        ...
        $class_name = Q::ini('app_config/APPID') . $class;
        if (!isset(self::$_metas[$class_name]))
        {
            self::$_metas[$class_name] = new QDB_ActiveRecord_Meta($class);
            self::$_metas[$class_name]->_init2();
        }
        return self::$_metas[$class_name];
    ...
    {
        $obj_id = 'activerecord_table_' . Q::ini('app_config/APPID') . '_' . strtolower($table_name);
