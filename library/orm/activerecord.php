                                                    ...
                                                    ArrayAccess
{
    ...
    {
        $this->_class_name = Q::ini('app_config/APPID') . '_' . get_class($this);
        if (! isset(self::$_meta[$this->_class_name]))
        {
            self::$_meta[$this->_class_name] = QDB_ActiveRecord_Meta::instance(get_class($this));
        }
    ...
    protected function _autofill($mode)
    {
        ...
        foreach ($fill_props as $prop => $fill)
        {
            ...
            {
                $this->_props[$prop] = date('Y-m-d H:i:s', time());
            }
            elseif ($fill === self::AUTOFILL_TIMESTAMP)
            {
                $this->_props[$prop] = intval(time());
            }
            elseif ($fill === self::AUTOFILL_DATE)
            {
                $this->_props[$prop] = date('Y-m-d', time());
            }
            elseif ($fill === self::AUTOFILL_TIME)
            {
                $this->_props[$prop] = date('H:i:s', time());
            }
    ...
    function import($data,$prop = array())
    {
        if($prop)
        {
            if(!is_array($prop))            $prop = explode(',',$prop);
            foreach($prop as $key)          if(isset($data[$key])) $this->$key = $data[$key];
        }else
        {
